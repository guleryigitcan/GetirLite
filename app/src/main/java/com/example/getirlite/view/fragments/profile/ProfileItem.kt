...
//            address -> R.drawable.ic_pin //TODO
    ...
    val imageVector: ImageVector
        get() = when (this) {
            userName, login -> Icons.Rounded.Person
            email -> Icons.Rounded.Mail
            phone ->Icons.Rounded.PhoneIphone
//            address -> R.drawable.ic_pin //TODO
            favourite -> Icons.Rounded.Favorite
    ...
    val route: String
        get() = when (this) {
            favourite -> Controllers.favouriteItems.name
            else -> { "" }
        }
    ...
    companion object {
        ...
        val buttons: List<ProfileItem> get() = if (User.didLogIn.bool) listOf(favourite, help, logout) else listOf(
            login, help
        )
    }
}